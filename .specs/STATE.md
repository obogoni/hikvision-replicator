...
### AD-032
- **Decision**: **A binary payload lives in its own table, and its owner carries a fingerprint instead of the bytes.** The aggregate holds hash, byte size and dimensions as owned columns (`UserConfiguration.cs:76-86`); the bytes sit in a dedicated table (`FacePictureConfiguration.cs:14`); and the navigation is **never auto-included** (`UserConfiguration.cs:103` — `AutoInclude(false)`). Every read answers from the owner's table alone. Any future binary payload follows this shape.
- **Reason**: **Resolves ROADMAP OD-4**, which asked how to stop ~10 GB of face images bloating the transactional database and slowing every query. Separating the table answers the query half outright: a get, a list and a conflict check never touch `face_pictures`, so the row width a catalogue page scans is independent of image size. The fingerprint on the owner is what makes that possible — change detection and idempotency (USR-26's byte-identical re-upsert must not advance `UpdatedAt`) need a hash, not the image.
- **Reason it resolves OD-4 differently from OD-4's own recommendation**: OD-4 proposed storing images **outside the database** in a filesystem or object store. That was rejected. Removal must tombstone the row and **destroy the biometric in the same transaction** (AD-034, USR-32) — an external store cannot participate in that transaction, so a crash between the two leaves an orphaned face with no owner, which is precisely the record that must not exist. At ~7.5 GB for 50,000 spectators the size is tolerable; an orphaned biometric is not. The bytes stay in PostgreSQL, and the query cost is solved by never joining rather than by moving the data.
- **Trade-off**: The database carries the volume anyway, so backup, restore and replication all pay for it — the win is query shape, not footprint, and this entry should not be read as claiming otherwise. Worse, **the guarantee is a convention, not a constraint**: nothing in the schema stops a later `.Include(u => u.Picture)` or an `AutoInclude(true)`, and the only thing standing between that and a 200 KB row on every catalogue page is the SQL-recording assertions in `UserPersistenceContractTests` (AD-036). The AD-036 Verifier confirmed those assertions are the *sole* killer of an `AutoInclude(true)` mutation — nothing else in 193 tests catches it.
- **Scope**: `Domain/FacePicture.cs`, `Domain/FaceFingerprint.cs`, `Infrastructure/UserConfiguration.cs`, `Infrastructure/FacePictureConfiguration.cs`, `Domain/Specs/**`. Phase 2 `replication-queue` inherits it: the fingerprint is what a device-side comparison reads. Proposed in `user-registry`'s `design.md` § Proposed project-level decisions and recorded here after the fact.
- **Date**: 2026-08-26
- **Status**: active — resolves OD-4

### AD-033
- **Decision**: **External-format constraints are normalized at the API boundary, not imposed on callers.** The conversion sits behind a port (`Shared/IFaceImageNormalizer.cs`) with a single adapter (`Infrastructure/SkiaFaceImageNormalizer.cs`), and the target envelope lives in **validated options** (`Infrastructure/FaceImageOptions.cs`), never in constants. SkiaSharp is the imaging adapter.
- **Reason**: Hikvision terminals accept a narrow envelope — JPEG, ≥ 640×480, **40–200 KB** — while integrators hold multi-megabyte phone photographs. Pushing that envelope onto every partner turns one server-side conversion into an integration burden repeated per partner, and makes the most common failure mode a rejected upload at the turnstile rather than a resize. Holding the envelope in options is what makes A-13's standing Phase 3 obligation — verify the band and the floor against real hardware — a configuration change instead of a code change.
- **Reason the 40 KB lower bound is not a typo**: over-compression is a device rejection cause, so an upper-bound-only implementation is wrong. This came from reading the DS-K1T606 documentation rather than assuming, and it is the kind of detail an envelope-in-constants design tends to lose.
- **Trade-off**: The conversion is **CPU-bound work on the latency path AD-014 makes the primary quality attribute**, which is why it is the one step carrying its own span and duration/size metrics (USR-40, USR-41) — the cost is accepted but must stay measured. The adapter choice also pins test evidence: golden hashes are recorded against SkiaSharp 3.119.4, so an upgrade **fails them by design** — review and re-record, never loosen. ImageSharp was rejected: v4 fails the build without a committed `sixlabors.lic`, and its free sample licence expires 2026-09-04.
- **Scope**: `Shared/IFaceImageNormalizer.cs`, `Infrastructure/SkiaFaceImageNormalizer.cs`, `Infrastructure/FaceImageOptions.cs`, `Features/Users/UpsertUser/**`. Proposed in `user-registry`'s `design.md` and recorded here after the fact.
- **Date**: 2026-08-26
- **Status**: active

### AD-034
- **Decision**: **An aggregate that a later phase must act on after its removal is tombstoned, not deleted.** Removal sets `DeletedAt DateTime?` (`UserConfiguration.cs:45`), the row stays and keeps its identity fields, and reads exclude it. The rider is the part that is easy to get wrong: **uniqueness that must survive deletion uses a full index; uniqueness that must not uses a partial one filtered on the active rows.**
  - `IX_users_ExternalRef` covers **every** row (`UserConfiguration.cs:108`), because resurrection has to find a tombstone by its key.
  - `IX_users_AccessCode` is partial on `"DeletedAt" IS NULL` (`UserConfiguration.cs:113-114`), because USR-06 scopes uniqueness to *active* users, so a removed spectator's PIN returns to the pool.
- **Reason**: Phase 2's Remove path needs a live replication target — something to enqueue a removal *against* — which a deleted row cannot provide. The tombstone also makes the integrator's own key stable across a remove/re-add cycle, which is what A-7's resurrection depends on. It does **not** mean keeping everything: the face bytes are destroyed in the same transaction as the tombstone, so Phase 2 inherits a live FK target and no biometric.
- **Reason the two indexes are deliberately asymmetric**: a single uniform rule contradicts one criterion or the other. Make `AccessCode` full and a removed spectator's PIN is reserved forever; make `ExternalRef` partial and resurrection takes the create branch and collides with a row it cannot see. Both directions were proved from both sides, and the AD-036 Verifier killed a mutation in each.
- **Trade-off**: Every read must remember the filter, and forgetting it is silent — a tombstoned spectator simply reappears. Rows accumulate with no purge path defined, which is deferred rather than solved. And the asymmetry is genuinely the most misreadable thing in the schema: it reads like an inconsistency until the two criteria are held side by side, which is why the index shapes are asserted from `pg_indexes` rather than trusted from the EF model.
- **Scope**: `Domain/User.cs` (`MarkDeleted`, `Restore`), `Infrastructure/UserConfiguration.cs`, `Domain/Specs/**`, `Features/Users/RemoveUser/**`. Phase 2 `replication-queue` inherits it directly. Proposed in `user-registry`'s `design.md` and recorded here after the fact.
- **Date**: 2026-08-26
- **Status**: active
...
- **Numbering**: this is **AD-035**, not AD-032, because AD-032, AD-033 and AD-034 were reserved by `user-registry`'s `design.md` (binary-payload table, normalize-at-the-boundary, tombstone + asymmetric index). They were written up on 2026-08-26 and now occupy the gap — the reservation held.
- **Date**: 2026-08-26
