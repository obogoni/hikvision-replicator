...
- **Next step**: **Merge the AD-025 amendment PR**, then specify Phase 1 item 2, `user-registry`. Resolve OD-4 (face-image storage — 10 GB of BLOBs in the transactional database) during that spec.
- **Blockers**: none.
...
- **Branch**: `docs/ad-025-mechanical-enforcement`, based on `main` (`9774e7a`). Everything else is merged and its branch deleted; `main` carries all four projects (`Api`, `Tests`, `IntegrationTests`, `E2E`), verified by reading `main` directly.
