...
**Status**: draft for confirmation · 2026-08-24
...
| A-15 | `ExternalRef` character set and comparison | Any non-blank string ≤ 255 chars, compared **case-sensitively** and byte-exactly | It is an opaque foreign identifier; imposing a format would reject valid integrator keys. Case-sensitive because folding could collide two distinct upstream identities — a silent, unrecoverable merge of two spectators. | **n** |
| A-16 | Repeated `DELETE` | Deleting an already-tombstoned user returns 204, not 404 | Makes `DELETE` idempotent so an integrator retry is safe, consistent with A-2's motivation. 404 is reserved for an `ExternalRef` that was never registered. | **n** |

**Open questions:** none — every item above is either confirmed or recorded with a chosen default
and rationale. A-7, A-12, A-13, A-15 and A-16 are agent-chosen defaults awaiting explicit
confirmation; A-13 additionally carries a Phase 3 verification obligation.
...
- WHEN `{externalRef}` contains URL-reserved characters THEN the decoded path segment SHALL be the identifier, so an `ExternalRef` containing `/` or `%` round-trips through `PUT`, `GET` and `DELETE`.
